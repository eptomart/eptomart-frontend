...
const TAB_LIST = ['dashboard', 'orders', 'pending-approval', 'alerts', 'cancelled-orders', 'sellers', 'seller-admins', 'categories', 'products', 'combos', 'product-approvals', 'daily-price', 'schedule', 'printer', 'bulk-harvest', 'news', 'refund-requests', 'wallets', 'users-cart', 'procurement', 'offers', 'reports', 'whatsapp-inbox', 'dev-settings'];
...
export default function KoyambeduAdmin() {
  ...
  const [costsForm,     setCostsForm]     = useState({ actualDeliveryCost: '', miscExpenses: '', transportCharge: '', packingCharge: '', costNote: '' });
  ...
  const saveCosts = async () => {
    ...
    try {
      await api.patch(`/koyambedu/admin/orders/${costsModal._id}/costs`, {
        ...
        transportCharge:    Number(costsForm.transportCharge) || 0,
        packingCharge:      Number(costsForm.packingCharge) || 0,
        costNote:           costsForm.costNote,
      });
      toast.success('Costs saved');
      setOrders(prev => prev.map(o => o._id === costsModal._id
        ? { ...o, adminCosts: { actualDeliveryCost: Number(costsForm.actualDeliveryCost)||0, miscExpenses: Number(costsForm.miscExpenses)||0, transportCharge: Number(costsForm.transportCharge)||0, packingCharge: Number(costsForm.packingCharge)||0, costNote: costsForm.costNote } }
        : o
      ));
      setCostsModal(null);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div style={{ background: 'linear-gradient(135deg,#14532d,#16a34a)' }} className="px-4 pt-8 pb-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-white font-black text-lg flex-1">Koyambedu Daily — Admin</h1>
          {isSuperAdmin && (
            <button onClick={() => navigate('/admin/koyambedu/inventory')}
              className="text-xs font-bold text-white bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-xl whitespace-nowrap">
              📦 Inventory &amp; Profit
            </button>
          )}
        </div>
      ...
      <div className="p-4">
                  ...
                  <div key={order._id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                        ...
                        <div className="flex gap-2 flex-wrap justify-end">
                          ...
                          <button onClick={() => { setCostsModal(order); setCostsForm({ actualDeliveryCost: order.adminCosts?.actualDeliveryCost || '', miscExpenses: order.adminCosts?.miscExpenses || '', transportCharge: order.adminCosts?.transportCharge || '', packingCharge: order.adminCosts?.packingCharge || '', costNote: order.adminCosts?.costNote || '' }); }}
                    ...
                    {isExp && (
                      <div className="border-t border-gray-100 divide-y divide-gray-50">
                        {/* Buyer delivery address + map pin — Super Admin only */}
                        {isSuperAdmin && (order.shippingAddress || order.buyerLocation) && (
                          <div className="px-4 py-2.5 bg-blue-50">
                            <p className="text-[10px] font-bold text-blue-700 uppercase mb-1.5 tracking-wide">📍 Buyer Delivery Address</p>
                            {order.shippingAddress && (
                              <p className="text-xs text-gray-700 leading-relaxed">
                                ...
                                {order.shippingAddress.phone && <span className="text-gray-500"> · {order.shippingAddress.phone}</span>}
                              </p>
                            )}
                            {order.buyerLocation && (
                              <div className="mt-1 flex items-center gap-2">
                                <span className="text-[10px] text-blue-600 font-semibold">
                                  📌 {order.buyerLocation.areaName || 'Pinned location'}
                                </span>
                                {order.buyerLocation.lat && order.buyerLocation.lng && (
                                  <a
                                    href={`https://maps.google.com/?q=${order.buyerLocation.lat},${order.buyerLocation.lng}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-[10px] text-white font-bold bg-blue-600 px-2 py-0.5 rounded-full"
                                  >
                                    Open in Maps ↗
                                  </a>
                                )}
                                {order.buyerLocation.lat && (
                                  <span className="text-[9px] text-gray-400">
                                    {order.buyerLocation.lat.toFixed(5)}, {order.buyerLocation.lng.toFixed(5)}
                                  </span>
                                )}
                              </div>
                            )}
        ...
        {/* ── NEWS (verified fruit/vegetable news, tab on/off) ── */}
        {tab === 'news' && (
          <NewsTab />
        )}
      ...
      {costsModal && (
        <div className="fixed inset-0 bg-black/50 z-[9997] flex items-end justify-center">
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-5 space-y-4" style={{ paddingBottom: 'calc(1.25rem + env(safe-area-inset-bottom))' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1 block">Transportation Charge (₹)</label>
                <input type="number" min="0" value={costsForm.transportCharge}
                  onChange={e => setCostsForm(f => ({ ...f, transportCharge: e.target.value }))}
                  placeholder="e.g. 50"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1 block">Packing Charge (₹)</label>
                <input type="number" min="0" value={costsForm.packingCharge}
                  onChange={e => setCostsForm(f => ({ ...f, packingCharge: e.target.value }))}
                  placeholder="e.g. 15"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500" />
              </div>
            </div>
            <p className="text-[10px] text-gray-400 -mt-2">Used by the Inventory &amp; Profit dashboard to compute customer-wise profit.</p>
            <div>
