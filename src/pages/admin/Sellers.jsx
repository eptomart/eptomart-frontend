              ...
              <div key={s._id} className="card p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
                    <FiUser size={17} className="text-primary-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-800 truncate">{s.businessName}</p>
                    <p className="text-xs text-gray-400 truncate">{s.contact?.email || s.user?.email || '—'}</p>
                    {s.sellerId && <p className="text-[10px] text-gray-400 font-mono">{s.sellerId}</p>}
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${STATUS_COLOR[s.status] || 'bg-gray-100 text-gray-600'}`}>
                    {s.status}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500">
                  <span>📞 {s.contact?.phone || s.user?.phone || '—'}</span>
                  <span>📍 {s.address?.city || '—'}</span>
                </div>
                  ...
                  {[
                    ...
                    <span key={key}
                      className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-full font-medium ${
                        s.kycStatus?.[key] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'
                      }`}>
                      {s.kycStatus?.[key] ? <FiCheckCircle size={10} /> : <FiXCircle size={10} />}
                      {label}
                    </span>
                  ))}
                </div>

                {/* Action buttons — full-width labeled row */}
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-gray-100">
                  <button onClick={() => navigate(`/admin/sellers/${s._id}/orders`)}
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-orange-50 text-orange-600 active:scale-95 transition-transform">
                    <FiShoppingBag size={13} /> View Orders
                  </button>
                  <button onClick={() => openEdit(s)}
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-blue-50 text-blue-600 active:scale-95 transition-transform">
                    <FiEdit2 size={13} /> Edit Details
                  </button>
                  {s.status === 'active' ? (
                    <button onClick={() => setStatus(s, 'suspended')}
                      className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-yellow-50 text-yellow-700 active:scale-95 transition-transform">
                      <FiToggleRight size={14} /> Suspend
                    </button>
                  ) : (
                    <button onClick={() => setStatus(s, 'active')}
                      className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-green-50 text-green-700 active:scale-95 transition-transform">
                      <FiToggleLeft size={14} /> Activate
                    </button>
                  )}
                  <button onClick={() => confirmDelete(s)} disabled={deleting === s._id}
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-red-50 text-red-600 active:scale-95 transition-transform disabled:opacity-40">
                    {deleting === s._id
