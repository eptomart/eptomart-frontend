  ...
  if (!showForm) {
    return (
      <div className="min-h-screen pb-24" style={{ background: '#F5F4F2' }}>
        <div className="flex items-center justify-between px-4 pt-10 pb-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-gray-900 font-bold text-lg">My Products</h1>
          </div>
        ...
        {loading && <div className="px-4 space-y-3 mt-4">{[1,2,3].map(i => <div key={i} className="h-20 rounded-2xl animate-pulse" style={{ background: 'rgba(0,0,0,0.03)' }} />)}</div>}
            ...
            <div key={p._id} className="rounded-2xl p-3" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)' }}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-400 text-xs capitalize">{p.category.replace('_', ' ')}</p>
              ...
              {editingStockId === p._id && (
                <div className="mt-3 pt-3 border-t border-gray-800 space-y-2">
                  <p className="text-gray-900 text-xs font-semibold">Update Daily Stock & Price</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-gray-400 text-[10px]">Stock ({p.unit})</label>
                      <input type="number" value={stockForm.stock} onChange={e => setStockForm(s => ({ ...s, stock: e.target.value }))} className="w-full px-2 py-1.5 rounded-lg text-gray-900 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-[10px]">Today's Price (₹)</label>
                      <input type="number" value={stockForm.todayPrice} onChange={e => setStockForm(s => ({ ...s, todayPrice: e.target.value }))} className="w-full px-2 py-1.5 rounded-lg text-gray-900 text-sm outline-none" style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }} />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setEditingStockId(null)} className="flex-1 py-1.5 rounded-lg text-xs" style={{ background: 'rgba(0,0,0,0.05)', color: '#fff' }}>Cancel</button>
                    <button onClick={() => updateStock(p._id)} className="flex-1 py-1.5 rounded-lg text-xs font-bold text-gray-900" style={{ background: '#f4941c' }}>Update</button>
                  </div>
                </div>
              )}
            </div>
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#F5F4F2' }}>
      <div className="flex items-center gap-3 px-4 pt-10 pb-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <button onClick={() => setShowForm(false)} className="p-2 rounded-full" style={{ background: 'rgba(0,0,0,0.05)' }}><FiArrowLeft className="text-gray-900" /></button>
        <h1 className="text-gray-900 font-bold text-lg">Add Product</h1>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div>
          ...
          <select value={form.category} onChange={e => setForm(v => ({ ...v, category: e.target.value }))}
            className="w-full px-3 py-2.5 rounded-xl text-sm text-gray-900 outline-none capitalize"
            style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {CUT_TYPES.map(c => (
              <button key={c} onClick={() => setForm(f => ({
                ...
                style={{ background: form.cutTypes.includes(c) ? '#f4941c' : 'rgba(0,0,0,0.05)', color: form.cutTypes.includes(c) ? '#fff' : 'rgba(255,255,255,0.5)' }}>
                {c.replace('_', ' ')}
              </button>
            ))}
          </div>
          ...
          {form.variants.map((v, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <input placeholder="Weight (g)" value={v.weight} onChange={e => updateVariant(i, 'weight', e.target.value)}
                className="w-24 px-2 py-1.5 rounded-lg text-gray-900 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }} />
              <input placeholder="Label (500g)" value={v.label} onChange={e => updateVariant(i, 'label', e.target.value)}
                className="flex-1 px-2 py-1.5 rounded-lg text-gray-900 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }} />
              <input placeholder="Price ₹" value={v.price} onChange={e => updateVariant(i, 'price', e.target.value)}
                className="w-20 px-2 py-1.5 rounded-lg text-gray-900 text-xs outline-none" style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '16px' }} />
            </div>
          ))}
        </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <label key={t.key} className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-gray-900 text-sm">{t.label}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <label className="cursor-pointer block w-full py-3 rounded-xl text-sm text-center" style={{ background: 'rgba(0,0,0,0.03)', border: '1px dashed rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.4)' }}>
        ...
        <button onClick={handleSubmit} disabled={saving} className="w-full py-3.5 rounded-2xl font-bold text-gray-900 disabled:opacity-60" style={{ background: '#f4941c' }}>
