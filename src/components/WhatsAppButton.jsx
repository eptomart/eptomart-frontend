...
import { openWhatsApp, openSupportWhatsApp } from '../utils/whatsapp';

/**
 * WhatsAppButton
 *
 * Props:
 *   phone       - target phone number (any format). If omitted, opens Eptomart support.
 *   message     - pre-filled message text
 *   label       - button label (default: 'Chat on WhatsApp')
 *   className   - extra Tailwind classes
 *   iconOnly    - render just the icon (for floating buttons)
 *   variant     - 'green' (default) | 'outline' | 'ghost'
 */
const WhatsAppButton = ({
  ...
  const handleClick = (e) => {
    ...
    if (phone) {
      openWhatsApp(phone, message, { onSuccess, onError });
    } else {
      openSupportWhatsApp(message || 'Hi Eptomart! I need help.');
      setLoading(false);
    }
  };

  const baseClass = 'inline-flex items-center gap-2 font-semibold rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-1 disabled:opacity-60 cursor-pointer';

  const variantClass = {
    green:   'bg-[#25D366] hover:bg-[#1ebe5a] text-white px-4 py-2 shadow-md hover:shadow-lg',
    outline: 'border-2 border-[#25D366] text-[#25D366] hover:bg-green-50 px-4 py-2',
    ghost:   'text-[#25D366] hover:bg-green-50 px-3 py-2',
  }[variant] || '';
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        viewBox="0 0 32 32"
        className={`flex-shrink-0 ${iconOnly ? 'w-6 h-6' : 'w-5 h-5'}`}
        fill="currentColor"
      >
